...
df = pd.read_csv('cluth_android_app_developers.csv')

profile = df['Profile']

# profile = profile.iloc[::-1]

reviewer_csv = open('reviewer_android_app_developers2.csv', 'w')
...
for i in profile:
	...
	for index,iCal in enumerate(total_classes):
		...
		try:
			...
			reviewer_tree = menu_review.find('div',attrs={'class':'col-24 reviewer-col pull-right hidden-xs'})
			...
			try:
				reviewer_role = reviewer_tree.find('div', attrs={'class':'group-fdb-reviewer'}).find('div', attrs={'class':'field field-name-field-fdb-title field-type-text field-label-hidden'}).find('div',attrs={'class':'field-item even'}).get_text()
			except:
				reviewer_role = 'None'

			try:
				reviewer_name = reviewer_tree.find('div', attrs={'class':'group-fdb-reviewer'}).find('div', attrs={'class':'field field-name-field-fdb-full-name-display field-type-text field-label-hidden'}).find('div',attrs={'class':'field-item even'}).get_text()
			except:
				reviewer_name = 'None'

			try:
				industry_type = reviewer_tree.find('div', attrs={'class':'group-fdb-interview'}).find('div', attrs={'class':'field field-name-field-fdb-user-industry field-type-text field-label-hidden field-label-inline clearfix'}).find('div',attrs={'class':'field-item even'}).get_text()
			except:
				industry_type = 'None'

			try:
				rev_company_size = reviewer_tree.find('div', attrs={'class':'group-fdb-interview'}).find('div', attrs={'class':'field field-name-field-fdb-company-size field-type-text field-label-hidden field-label-inline clearfix'}).find('div',attrs={'class':'field-item even'}).get_text()
			except:
				rev_company_size = 'None'

			try:
				reviewer_location = reviewer_tree.find('div', attrs={'class':'group-fdb-interview'}).find('div', attrs={'class':'field field-name-field-fdb-location field-type-text field-label-hidden field-label-inline clearfix'}).find('div',attrs={'class':'field-item even'}).get_text()
			except:
